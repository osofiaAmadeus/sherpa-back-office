...
import com.amadeus.sherpa.backoffice.config.KafkaProperties;
...
class SherpaBackOfficeKafkaResourceIT {
    ...
    void setup() {
        ...
        SherpaBackOfficeKafkaResource kafkaResource = new SherpaBackOfficeKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/sherpa-back-office-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/sherpa-back-office-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
